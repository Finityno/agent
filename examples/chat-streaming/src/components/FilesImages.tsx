...
export default function Example() {
  ...
  const handleSubmitFileQuestion = useCallback(
    ...
    [sendMessage, file?.fileId, threadId],
  );

  const handleImageGenerated = useCallback(
    async (imageData: { storageId: string; url: string }) => {
      setFile({ fileId: imageData.storageId, url: imageData.url });
      setActiveTab("upload");
    },
    [],
  );
        ...
        <main className="flex-1 flex items-center justify-center p-8">
          <div className="w-full max-w-xl mx-auto flex flex-col items-center gap-6 bg-white rounded-2xl shadow-lg p-8 border border-gray-200">
            ...
            {messages.results?.length > 0 ? (
              ...
              <Tabs value={activeTab} onValueChange={(v: string) => setActiveTab(v as "upload" | "generate")} className="w-full">
